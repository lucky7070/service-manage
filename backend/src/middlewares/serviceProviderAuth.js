...
export const requireServiceProviderAuth = async (req, res, next) => {
    try {
        ...
        const serviceProvider = await ServiceProvider.findOne({ _id: payload.id, deletedAt: null, isActive: true }, "_id name mobile email image serviceCategoryId profileStatus").lean();
