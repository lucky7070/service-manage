...
export default function FranchiseProviderWorkPhotosPage() {
    const { id } = useParams();
    ...
    const getData = useCallback(async () => {
        if (!id) return;
        setLoading(true);
        const { data } = await AxiosHelperFranchise.getData(`/service-providers/${id}`);
        if (data.status && data.data) {
            setProviderName(String(data.data?.name ?? ""));
            setPhotos(Array.isArray(data.data.photos) ? data.data.photos : []);
            setLoading(false);
        } else {
    ...
    const uploadFiles = async (fileList: File[]) => {
        ...
        const { data } = await AxiosHelperFranchise.postData(`/service-providers/${id}/photos`, formData, true);
        setUploading(false);
        if (data.status) {
    ...
    const removePhoto = async (photo: WorkPhoto) => {
        ...
        const { data } = await AxiosHelperFranchise.deleteData(`/service-providers/${id}/photos/${photo._id}`);
        if (data.status) {
    ...
    const persistOrder = async (next: WorkPhoto[]) => {
        ...
        const { data } = await AxiosHelperFranchise.putData(`/service-providers/${id}/photos/reorder`, { orderedIds }, false);
        if (data.status) {
            setPhotos(Array.isArray(data.data) ? data.data : next);
        } else {
    ...
    return (
        <section className="space-y-4">
            <AdminPageHeader
                title="Work Photos"
                subtitle={loading ? "Loading…" : providerName ? `Portfolio images for ${providerName}.` : "Portfolio images for this service provider."}
                action={
            ...
            <FileDropzone accept="image/*" multiple uploading={uploading} disabled={uploading} onFilesSelected={(files) => void uploadFiles(files)}>
                {({ openFileDialog }) => (
                    <p>
                        Drag and drop images here, or{" "}
                        <span role="button" tabIndex={0} className="font-medium text-primary-600 underline hover:text-primary-700 dark:text-primary-400" onClick={openFileDialog}>
                            Browse
                        </span>
                    </p>
                )}
            ...
            <div className="rounded-2xl border border-indigo-100 bg-white p-4 dark:border-slate-700 dark:bg-slate-900/80">
                {loading ? (
                    ...
                    <SortableList
                        ...
                        onEnd={persistOrder}
                        renderItem={(p) => (
                            <li key={p._id} className="group relative overflow-hidden rounded-xl border border-indigo-100 bg-slate-50 dark:border-slate-600 dark:bg-slate-800">
                                ...
                                <div className="drag-handle absolute left-2 top-2 cursor-grab rounded-md bg-white/90 p-1 shadow active:cursor-grabbing dark:bg-slate-900/90" title="Drag to Reorder">
                                    <GripVertical className="h-4 w-4 text-slate-600" aria-hidden />
                                </div>
                                <div className="absolute right-2 top-2">
                                    <Button type="button" size="sm" variant="danger" className="opacity-90 shadow" title="Remove" aria-label="Remove Photo" onClick={(e) => { e.stopPropagation(); void removePhoto(p); }}>
