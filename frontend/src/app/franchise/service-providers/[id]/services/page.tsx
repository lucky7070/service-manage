...
type ServiceTypeOption = SelectOption & { basePrice?: number | null };
...
export default function FranchiseProviderServicesPage() {
    const { id } = useParams();
    ...
    const getData = useCallback(async (isLoading = true) => {
        ...
        const [{ data }, { data: providerData }] = await Promise.all([
            AxiosHelperFranchise.getData(`/service-providers/${id}/services`),
            AxiosHelperFranchise.getData(`/service-providers/${id}`)
        ]);
        if (data.status && data.data?.provider && data.data.record) {
            ...
            const ids = Array.isArray(detail?.serviceCategoryIds) && detail.serviceCategoryIds.length
                ...
                : [String(data.data.provider.serviceCategoryId || detail?.serviceCategoryId || "")].filter(Boolean);
    ...
    const loadServiceTypeOptions = useCallback(async (inputValue: string): Promise<ServiceTypeOption[]> => {
        ...
        const { data } = await AxiosHelperFranchise.getData("/service-types", { categoryId: selectedCategoryId, query: inputValue || "", limit: 20 });
        if (data.status && Array.isArray(data.data)) {
            return data.data.map((row: { _id: string; name: string; basePrice?: number | null }) => ({
                value: row._id,
                label: row.name,
    ...
    const remove = async (row: ProviderServiceRow) => {
        ...
        const { data } = await AxiosHelperFranchise.deleteData(`/service-providers/${id}/services/${row._id}`);
        if (data.status) {
            ...
            <Modal show={!!open} onClose={() => setOpen(null)} title={open === "edit" ? "Update Provider Service" : "Add Provider Service"} size="lg">
                <Formik
                    ...
                    onSubmit={async (values, { setSubmitting, setErrors }) => {
                        ...
                        const { data } = open === "edit"
                            ? await AxiosHelperFranchise.putData(`/service-providers/${id}/services/${values._id}`, payload)
                            : await AxiosHelperFranchise.postData(`/service-providers/${id}/services`, payload);
                        if (data.status) {
                        ...
                        <Form className="space-y-4">
                            {providerCategoryIds.length > 1 ? (
                                <div className="space-y-2">
                                    <Label htmlFor="franchise-provider-service-category">Service Category</Label>
                                    <Select
                                        id="franchise-provider-service-category"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="franchise-provider-service-type">Service Type</Label>
                                <AsyncSelect
                                    inputId="franchise-provider-service-type"
                                    ...
                                    onChange={(option) => {
                                        ...
                                        if (!values.price && next?.basePrice != null) setFieldValue("price", String(next.basePrice));
                                    }}
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="franchise-provider-service-price">Provider Price</Label>
                                    <Field as={Input} id="franchise-provider-service-price" name="price" type="number" min={0} />
                                    <ErrorMessage className="text-xs text-rose-600" name="price" component="small" />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="franchise-provider-service-status">Status</Label>
                                    <Field as={Select} id="franchise-provider-service-status" name="status">
