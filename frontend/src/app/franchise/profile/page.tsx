...
const passwordValidationSchema = Yup.object().shape({
    current_password: Yup.string().required("Current password is required.").min(8, "Current password must be at least 8 characters.").max(50, "Current password must be at most 50 characters."),
    ...
    confirm_password: Yup.string().required("Please confirm your new password.").oneOf([Yup.ref("new_password")], "New password and confirmation do not match.")
});

export default function FranchiseProfilePage() {
    const router = useRouter();
    const dispatch = useAppDispatch();
    const [profile, setProfile] = useState<FranchiseProfile>({ name: "", mobile: "", email: "" });
    const [selectedImage, setSelectedImage] = useState<File | null>(null);
    const [imagePreview, setImagePreview] = useState<string | null>(null);
    const avatarLetter = (profile.name?.charAt(0) || "F").toUpperCase();
    ...
    useEffect(() => {
        ...
        (() => {
            if (selectedImage) {
                ...
                setImagePreview(resolveFileUrl(profile.image ?? null));
            }
        })();
    ...
    const updateProfileImage = async () => {
        ...
        const { data } = await AxiosHelperFranchise.putData("/profile/image", formData, true);
        if (data.status) {
    ...
    return (
        <section className="space-y-4">
            <AdminPageHeader title="Profile" subtitle="Manage your franchise account details." />
            <div className="grid gap-4 grid-cols-1 lg:grid-cols-5">
                <div className="lg:col-span-1">
                    <div className="flex h-full flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <Label htmlFor="profile-image" className="mb-0! group relative flex h-24 w-24 cursor-pointer items-center justify-center rounded-full transition-all hover:scale-[1.02]">
                            ...
                            <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-slate-900 text-3xl font-semibold text-white ring-1 ring-slate-700/60 dark:border-slate-700">
                                {imagePreview ? <Image src={imagePreview} alt="Profile preview" className="h-full w-full object-cover" /> : <span className="select-none">{avatarLetter}</span>}
                            </div>
                        </Label>
                ...
                <div className="lg:col-span-2">
                    <Formik
                        ...
                        onSubmit={async (values, { setSubmitting, setErrors }) => {
                            const { data } = await AxiosHelperFranchise.putData("/profile", values);
                            if (data.status) {
                                toast.success(data.message);
                                setProfile(data.data);
                                dispatch(updateFranchise(data.data));
                ...
                <div className="lg:col-span-2">
                    <Formik
                        ...
                        onSubmit={async (values, { setSubmitting, setErrors, resetForm }) => {
                            const { data } = await AxiosHelperFranchise.putData("/profile/password", values);
                            if (data.status) {
                                toast.success(data.message);
                                resetForm();
                            } else {
