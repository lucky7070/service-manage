...
export default function FranchiseLoginPage() {
    ...
    return (
        <div className="relative min-h-screen flex items-center justify-center p-2 lg:p-4 bg-linear-to-br from-[#f3f7ff] via-white to-[#e8efff] dark:from-[#0b1020] dark:via-[#111a2f] dark:to-[#172443]">
            ...
            <div className="mx-auto grid max-w-4xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900 md:grid-cols-2">
                <div className="bg-linear-to-br from-slate-900 via-slate-800 to-indigo-900 p-8 text-white">
                    ...
                    <h1 className="mt-3 text-3xl font-semibold">Franchise Portal</h1>
                    <p className="mt-3 text-sm text-slate-200">Secure login for franchise partners to manage service providers and account details.</p>
                    <div className="mt-8 rounded-xl border border-white/20 bg-white/10 p-4 text-sm">
                        <p>Use your franchise email/mobile and password to continue.</p>
                    </div>
                </div>
                <div className="p-8">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Login</h2>
                    <p className="mb-4 mt-1 text-sm text-slate-600 dark:text-slate-300">Sign in to your franchise account.</p>
                    <Formik
                        initialValues={{ identifier: "", password: "" }}
                        validationSchema={schema}
                        onSubmit={async (values, { setSubmitting }) => {
                            const { data } = await AxiosHelperFranchise.postData("/login", values);
                            if (data.status) {
