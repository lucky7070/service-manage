...
import { Button, Input, Label } from "@/components/front/ui";
import "leaflet/dist/leaflet.css";

const AddressLocationMap = dynamic(() => import("@/components/admin/AddressLocationMap"), {
    ssr: false,
    loading: () => <div className="flex h-72 items-center justify-center rounded-xl border border-border bg-muted text-sm text-muted-foreground">Loading map...</div>
});
...
export default function AddressLocationPicker({ latitude, longitude, searchHint = "", onChange }: AddressLocationPickerProps) {
    ...
    const searchLocation = async () => {
        ...
        try {
            ...
            if (!Array.isArray(data) || data.length === 0) setError("No location found. Try a more specific address.");
        } catch {
        ...
        <div className="space-y-3 rounded-xl border border-border bg-muted/30 p-3">
            <div className="space-y-2">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                    ...
                    />
                    <Button type="button" variant="outline" onClick={() => void searchLocation()} disabled={loading}>
                ...
                <div className="space-y-1 rounded-lg border border-border bg-background p-2">
                    {results.map((result) => (
                        <button
                            ...
                            type="button"
                            className="flex w-full items-start gap-2 rounded-md px-2 py-2 text-left text-sm text-foreground hover:bg-muted"
            ...
            <AddressLocationMap latitude={lat} longitude={lng} onChange={(nextLat, nextLng) => onChange(formatCoordinate(nextLat), formatCoordinate(nextLng))} />
            <p className="text-xs text-muted-foreground">Search a location, then drag the marker or click the map to adjust exact coordinates.</p>
        </div>
    );
}
